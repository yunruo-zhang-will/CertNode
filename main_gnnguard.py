...
def build_guard_model(args, num_x, num_labels):
    common_kwargs = dict(
        hidden_size=args.n_hidden,
        guard_threshold=args.guard_threshold,
        guard_temperature=args.guard_temperature,
        guard_memory=args.guard_memory,
    )
    if args.model == 'GCN':
        return NodeGCNGNNGuard(num_x, num_labels, **common_kwargs).to(args.device)
    if args.model == 'GAT':
        return NodeGATGNNGuard(num_x, num_labels, **common_kwargs).to(args.device)
    if args.model == 'GSAGE':
        return NodeGSAGEGNNGuard(num_x, num_labels, **common_kwargs).to(args.device)
    if args.model == 'GIN':
        return NodeGINGNNGuard(num_x, num_labels, **common_kwargs).to(args.device)
    if args.model == 'APPNP':
        return NodeAPPNPGNNGuard(num_x, num_labels, **common_kwargs, K=args.appnp_k, alpha=args.appnp_alpha).to(args.device)
...
def load_checkpoint(args, num_x, num_labels):
    ...
    if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
        model = build_guard_model(args, num_x, num_labels)
...
def train_guard_model(model, features, edge_index, labels, train_mask, val_mask, optimizer, args, num_x, num_labels):
    endure_count = 0
    best_acc_val = 0.0
    ...
    with open(train_result_path, 'w') as txt_file:
        txt_file.write(f'defense_method: gnnguard\n')
        txt_file.write(f'best_acc_val: {best_acc_val:.6f}\n')
        txt_file.write(f'guard_threshold: {args.guard_threshold:.6f}\n')
        txt_file.write(f'guard_temperature: {args.guard_temperature:.6f}\n')
        txt_file.write(f'guard_memory: {args.guard_memory:.6f}\n')
...
def save_attack_results(output_dir, attack_method, attack_eval, attack_meta, attack_info, attacked_features, attacked_edge_index, clean_edge_index, attack_seconds, clean_loss, clean_acc, args):
    ...
    with open(csv_path, mode='w', newline='') as csv_file:
        ...
        csv_writer.writerow(['defense_method', 'gnnguard'])
        ...
        csv_writer.writerow(['guard_threshold', args.guard_threshold])
        csv_writer.writerow(['guard_temperature', args.guard_temperature])
        csv_writer.writerow(['guard_memory', args.guard_memory])
    ...
    with open(info_path, 'w') as info_file:
        info_file.write(f'defense_method: gnnguard\n')
        ...
        info_file.write(f'guard_threshold: {args.guard_threshold}\n')
        info_file.write(f'guard_temperature: {args.guard_temperature}\n')
        info_file.write(f'guard_memory: {args.guard_memory}\n')
...
def prepare_output_paths(args):
    ...
    default_model_dir = os.path.join(
        args.model_dir,
        f'{args.dataset}_{args.model}_gnnguard_seed{args.seed}',
        'model.pth',
    )
    ...
    args.output_dir = os.path.join(
        args.output_dir,
        f'gnnguard_{attack_tag}_{args.dataset}_{args.model}_seed{args.seed}_{datetime.now().strftime("%Y%m%d_%H%M%S")}',
    )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='GNNGuard defense training and robustness evaluation for node classification GNNs')
    ...
    parser.add_argument('-guard_threshold', type=float, default=0.1, help='minimum cosine-similarity gate kept by GNNGuard')
    parser.add_argument('-guard_temperature', type=float, default=1.0, help='temperature for similarity sharpening')
    parser.add_argument('-guard_memory', type=float, default=0.5, help='blend weight for previous-layer edge attention')
    parser.add_argument('-appnp_k', type=int, default=10, help='APPNP propagation steps')
    parser.add_argument('-appnp_alpha', type=float, default=0.1, help='APPNP teleport probability')

    parser.add_argument('-output_dir', type=str, default='./results_gnnguard', help='output directory')
    parser.add_argument(
        '-model_dir',
        '-model_root',
        dest='model_dir',
        type=str,
        default='./models_gnnguard',
        help='model root directory; checkpoint path resolves to <model_dir>/<dataset>_<model>_gnnguard_seed<seed>/model.pth',
    )
